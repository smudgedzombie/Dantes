...
import { BellRing, Flame, Sparkles, ArrowRight, Wallet } from "lucide-react";
import { Link } from "wouter";

// Fallback mock data in case the API isn't fully ready
const MOCK_SUMMARY = {
  totalAlerts: 12,
  triggeredToday: 3,
  totalSavings: 4500,
...
const MOCK_TRENDING = [
  { id: 1, title: "Sony WH-1000XM5 Wireless Noise Cancelling Headphones", platform: "shopee", originalPrice: 14990, salePrice: 10990, discountPercent: 26, category: "Electronics", isTrending: true, isVerified: true, url: "#" },
  { id: 2, title: "Dyson V12 Detect Slim Submarine", platform: "lazada", originalPrice: 31900, salePrice: 24900, discountPercent: 21, category: "Home Appliances", isTrending: true, isVerified: true, url: "#" },
  { id: 3, title: "Nintendo Switch OLED Model White", platform: "shopee", originalPrice: 14990, salePrice: 11290, discountPercent: 24, category: "Gaming", isTrending: true, isVerified: false, url: "#" },
  { id: 4, title: "Apple AirPods Pro 2nd Gen", platform: "tiktok", originalPrice: 8990, salePrice: 6590, discountPercent: 26, category: "Electronics", isTrending: true, isVerified: true, url: "#" },
];

export default function Home() {
  const { data: summaryData, isLoading: isLoadingSummary } = useGetAlertsSummary?.() || { data: null, isLoading: false };
  const { data: trendingData, isLoading: isLoadingTrending } = useGetTrendingDeals?.() || { data: null, isLoading: false };
  const { data: matchedData, isLoading: isLoadingMatched } = useGetMatchedDeals?.() || { data: null, isLoading: false };

  const summary = summaryData || MOCK_SUMMARY;
  const trendingDeals = trendingData || MOCK_TRENDING;
  const matchedDeals = matchedData || [];
    ...
    <div className="space-y-8 pb-10">
        ...
        <Card className="bg-primary text-primary-foreground border-none p-5 shadow-lg shadow-primary/20 relative overflow-hidden group">
          ...
          <div className="flex items-center gap-4 relative z-10">
            ...
            <div>
              ...
              <p className="text-2xl font-black">{formatPrice(summary.totalSavings)}</p>
            </div>
          </div>
        </Card>
        ...
        <Card className="bg-secondary text-secondary-foreground border-none p-5 shadow-lg shadow-secondary/20 relative overflow-hidden group">
          ...
          <div className="flex items-center gap-4 relative z-10">
            ...
            <div>
              ...
              <p className="text-2xl font-black">{summary.triggeredToday} items</p>
            </div>
          </div>
        </Card>
        ...
        <Card className="bg-card border-border p-5 shadow-sm flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-black text-foreground">{summary.matchedCount} deals</p>
            </div>
          </div>
      ...
      {matchedDeals.length > 0 && (
        <section className="space-y-6 pt-6 border-t border-border">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {matchedDeals.map((match: any, i: number) => (
