...
export function BgVariant({ cfg }: { cfg: BgConfig }) {
  return (
    <div style={{ minHeight: "100vh", background: cfg.bg, color: "white", fontFamily: "system-ui, sans-serif", overflowX: "hidden", position: "relative" }}>
      <div style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 0, backgroundImage: `linear-gradient(${cfg.gridLine} 1px,transparent 1px),linear-gradient(90deg,${cfg.gridLine} 1px,transparent 1px)`, backgroundSize: "60px 60px" }} />

      {/* Label badge */}
      <div style={{ position: "fixed", top: 12, right: 12, zIndex: 999, background: "#FFB300", color: "#080808", fontSize: 10, fontWeight: 900, padding: "4px 10px", borderRadius: 2, letterSpacing: "0.15em", fontFamily: "monospace" }}>
        {cfg.label} — {cfg.hex}
      </div>

      {/* Nav */}
      <nav style={{ position: "relative", zIndex: 20, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "20px 32px", borderBottom: `1px solid ${cfg.borderBase}` }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
          ...
          <button style={{ padding: "8px 20px", background: "#FFB300", color: "#080808", fontSize: 11, fontFamily: "monospace", fontWeight: 700, border: "none", borderRadius: 2, letterSpacing: "0.15em", cursor: "pointer", boxShadow: "0 0 24px rgba(255,179,0,0.30)" }}>
            AUTHORIZE ACCESS
          </button>
        </div>
      </nav>
      ...
      <section style={{ position: "relative", zIndex: 10, minHeight: "88vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", padding: "0 24px 48px" }}>
        <div style={{ position: "relative", zIndex: 20, display: "flex", flexDirection: "column", alignItems: "center" }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 12, padding: "8px 16px", border: "1px solid rgba(255,107,0,0.50)", background: "rgba(255,107,0,0.09)", borderRadius: 2, marginBottom: 32 }}>
          ...
          <div style={{ marginBottom: 12 }}>
            <span style={{ fontSize: 9, fontFamily: "monospace", color: "rgba(255,179,0,0.68)", letterSpacing: "0.4em" }}>THAILAND-BORN · GLOBALLY DEPLOYED</span>
          </div>
          ...
          <p style={{ color: cfg.bodyText, maxWidth: 640, fontSize: 15, lineHeight: 1.7, marginBottom: 24 }}>
          ...
          <div style={{ display: "flex", gap: 24, marginBottom: 40, fontSize: 10, fontFamily: "monospace", color: cfg.subText, flexWrap: "wrap", justifyContent: "center" }}>
            <span>◆ Bangkok HQ</span>
            <span style={{ color: "rgba(255,179,0,0.20)" }}>|</span>
            <span>◆ Thai Enterprise Clients</span>
            <span style={{ color: "rgba(255,179,0,0.20)" }}>|</span>
            <span>◆ THB · USD · Multi-currency</span>
          </div>
          <button style={{ padding: "14px 32px", background: "#FFB300", color: "#080808", fontFamily: "monospace", fontWeight: 700, fontSize: 13, border: "none", borderRadius: 2, letterSpacing: "0.15em", cursor: "pointer", boxShadow: "0 0 40px rgba(255,179,0,0.42)" }}>
      ...
      {/* Stats bar */}
      <div style={{ position: "relative", zIndex: 10, borderTop: `1px solid ${cfg.borderBase}`, borderBottom: `1px solid ${cfg.borderBase}`, padding: "20px 32px" }}>
        <div style={{ maxWidth: 900, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 24, textAlign: "center" }}>
          {[
            ...
            <div key={s.label}>
              ...
              <div style={{ color: cfg.subText, fontSize: 10, fontFamily: "monospace" }}>{s.sub}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <section style={{ position: "relative", zIndex: 10, maxWidth: 900, margin: "0 auto", padding: "80px 32px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 24 }}>
          {[
            ...
            <div key={item.title} style={{ border: `1px solid ${cfg.borderBase}`, background: cfg.cardBg, borderRadius: 2, padding: 28 }}>
              <div style={{ fontSize: 28, marginBottom: 16 }}>{item.icon}</div>
              <h3 style={{ color: "white", fontWeight: 700, fontSize: 17, marginBottom: 12 }}>{item.title}</h3>
              <p style={{ color: cfg.bodyText, fontSize: 13, lineHeight: 1.7, margin: 0 }}>{item.body}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ position: "relative", zIndex: 10, maxWidth: 1100, margin: "0 auto", padding: "0 32px 48px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16, marginBottom: 24 }}>
          {AGENTS.map(a => (
            <div key={a.code} style={{ border: `1px solid ${cfg.borderBase}`, background: cfg.cardBg, borderRadius: 2, padding: 20 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
                <div>
                  ...
                  <div style={{ color: cfg.bodyText, fontSize: 11, fontFamily: "monospace" }}>{a.industry}</div>
                </div>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                {a.modules.map(m => (
                  <span key={m} style={{ fontSize: 10, fontFamily: "monospace", padding: "2px 8px", background: cfg.bg, border: `1px solid ${cfg.borderBase}`, color: cfg.bodyText, borderRadius: 2 }}>{m}</span>
                ))}
              </div>
            </div>
          ))}
      ...
      <section style={{ position: "relative", zIndex: 10, maxWidth: 1100, margin: "0 auto", padding: "0 32px 80px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16 }}>
          {MODULES.map(m => (
            <div key={m.id} style={{ border: `1px solid ${cfg.borderBase}`, background: cfg.cardBg, borderRadius: 2, padding: 20 }}>
              ...
              <div style={{ color: cfg.bodyText, fontSize: 12, lineHeight: 1.6 }}>{m.desc}</div>
            </div>
          ))}
        </div>
      </section>

      <footer style={{ position: "relative", zIndex: 10, borderTop: `1px solid ${cfg.borderBase}`, padding: "32px", textAlign: "center" }}>
        <div style={{ color: cfg.subText, fontSize: 10, fontFamily: "monospace", letterSpacing: "0.3em" }}>© 2026 THE BLOOM SOCIETY · DANTÈS FINANCE · ALL RIGHTS RESERVED</div>
      </footer>
    </div>
  );
}
