...
export function CyberGold() {
  ...
  return (
    <div className="min-h-screen bg-[#080808] text-white overflow-hidden relative selection:bg-[#FFB300] selection:text-black">
      {/* Scanline Overlay */}
      <div className="pointer-events-none fixed inset-0 z-50 opacity-[0.03]" style={{ background: "linear-gradient(transparent 50%, rgba(0, 0, 0, 0.25) 50%), linear-gradient(90deg, rgba(255, 179, 0, 0.06), rgba(255, 107, 0, 0.02), rgba(255, 179, 0, 0.06))", backgroundSize: "100% 2px, 3px 100%" }} />
      
      {/* CSS overrides for fonts */}
      <style>{`
        .font-rajdhani { font-family: 'Rajdhani', sans-serif; }
        .font-tech { font-family: 'Share Tech Mono', monospace; }
        .glow-text-amber { text-shadow: 0 0 10px rgba(255, 179, 0, 0.5); }
        .glow-text-orange { text-shadow: 0 0 10px rgba(255, 107, 0, 0.5); }
        .glow-border-amber { box-shadow: 0 0 15px rgba(255, 179, 0, 0.2), inset 0 0 10px rgba(255, 179, 0, 0.1); border-color: rgba(255, 179, 0, 0.5); }
        .glow-border-orange { box-shadow: 0 0 15px rgba(255, 107, 0, 0.2), inset 0 0 10px rgba(255, 107, 0, 0.1); border-color: rgba(255, 107, 0, 0.5); }
      ...
      {/* Ticker */}
      <div className="absolute top-0 left-0 w-full bg-black/80 border-b border-[#FFB300]/20 py-1.5 z-40 overflow-hidden font-tech text-xs tracking-wider text-[#FFB300]/80 flex">
      ...
      <section className="relative pt-24 pb-32 px-8 max-w-7xl mx-auto flex flex-col items-center justify-center text-center z-10 min-h-[80vh]">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(255,179,0,0.05)_0%,transparent_50%)]" />
        
        <div className="inline-block border border-[#FFB300]/30 px-4 py-1.5 mb-8 font-tech text-sm text-[#FFB300] tracking-widest bg-black/50 backdrop-blur-sm">
          SYS.STATUS: <span className="text-[#FFB300] glow-text-amber">ONLINE</span>
        </div>
        
        <h1 className="font-tech text-7xl md:text-9xl font-bold tracking-tighter mb-4 text-white glow-text-amber mix-blend-screen">
          DANTÈS
        </h1>
        
        <div className="font-rajdhani text-2xl md:text-4xl text-[#FF6B00] tracking-[0.2em] mb-12 h-10 glow-text-orange font-semibold uppercase">
        ...
        <div className="flex flex-col sm:flex-row gap-6 font-tech tracking-widest">
          <button className="bg-[#FFB300]/10 border border-[#FFB300] text-[#FFB300] px-8 py-4 glow-border-amber hover:bg-[#FFB300] hover:text-black transition-all flex items-center justify-center gap-3 group">
          ...
          {[
            ...
            <div key={i} className="p-8 flex flex-col items-center md:items-start text-center md:text-left relative group">
              ...
              <div className="flex items-center gap-2 mb-2 font-tech text-sm text-white/40 tracking-widest">
                <div className={`w-2 h-2 rounded-full ${stat.alert ? 'bg-[#FF6B00]' : 'bg-[#FFB300]'} animate-pulse`} />
                {stat.label}
              </div>
      ...
      <section className="relative z-10 py-32 px-8 max-w-7xl mx-auto">
        <div className="flex flex-col items-center mb-16">
          <div className="font-tech text-[#FFB300] text-sm tracking-widest mb-4 flex items-center gap-2">
            <CornerDownRight className="w-4 h-4" /> MODULE.01
          </div>
          <h2 className="font-rajdhani text-4xl md:text-5xl font-bold tracking-widest uppercase">GRAHAM <span className="text-[#FF6B00]">AGENTS</span></h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            {
              title: "ALPHA SCOUT",
              icon: <Zap className="w-8 h-8 text-[#FFB300]" />,
              desc: "Autonomous market scanning across SET & crypto. Identifies asymmetrical opportunities before they trend.",
              status: "ACTIVE"
            },
            {
              title: "RISK SENTINEL",
              icon: <ShieldAlert className="w-8 h-8 text-[#FF6B00]" />,
              desc: "24/7 portfolio protection. Executes dynamic hedging strategies when volatility spikes.",
              status: "ACTIVE"
            },
            {
              title: "PORTFOLIO ARCHITECT",
              icon: <Cpu className="w-8 h-8 text-[#FFB300]" />,
              desc: "Deep-learning driven asset allocation tailored to your specific risk-return profile.",
              status: "TRAINING"
            }
          ].map((agent, i) => (
            <div key={i} className="bg-black/60 border border-white/10 p-8 hover:border-[#FFB300]/50 transition-colors relative group overflow-hidden">
              <div className="absolute top-0 right-0 p-4 font-tech text-[10px] tracking-widest text-white/30">
                SYS_ID: 00{i + 1}
              </div>
              <div className="mb-6 p-4 bg-white/5 inline-block rounded-sm group-hover:glow-border-amber border border-transparent transition-all">
                {agent.icon}
              </div>
              <h3 className="font-rajdhani text-2xl font-semibold tracking-wider mb-4 text-white">{agent.title}</h3>
              ...
              <div className="flex items-center justify-between border-t border-white/10 pt-4 font-tech text-xs tracking-widest">
                ...
                <span className={agent.status === 'ACTIVE' ? 'text-[#FFB300]' : 'text-[#FF6B00]'}>{agent.status}</span>
              </div>
            </div>
          ))}
        </div>
            ...
            {[
              ...
              <div key={i} className="relative p-[1px] bg-gradient-to-b from-white/20 to-transparent group">
                <div className="bg-[#080808] p-10 h-full">
                  <div className="font-tech text-4xl text-[#FFB300]/20 mb-6">"</div>
                  <p className="font-rajdhani text-xl text-white/80 leading-relaxed mb-8 font-light">
                    {testimonial.quote}
                  </p>
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 bg-white/5 border border-white/10 flex items-center justify-center font-tech text-sm text-[#FF6B00]">
                      {testimonial.author.split('')[0]}
                    </div>
                    <div>
                      <div className="font-tech text-sm tracking-widest text-white">{testimonial.author}</div>
                      <div className="font-tech text-xs tracking-widest text-[#FFB300]/60">{testimonial.role}</div>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <section className="relative z-10 py-32 px-8 max-w-3xl mx-auto text-center">
        ...
        <h2 className="font-rajdhani text-4xl md:text-6xl font-bold tracking-widest uppercase mb-8 text-white">
          INITIALIZE <span className="text-[#FFB300]">SEQUENCE</span>
        </h2>
        ...
        <form className="flex flex-col md:flex-row gap-4 mb-8 font-tech">
          <input 
            ...
            className="flex-1 bg-black border border-white/20 px-6 py-4 text-white placeholder-white/30 focus:outline-none focus:border-[#FFB300] transition-colors tracking-widest"
          />
          <button type="button" className="bg-[#FFB300] text-black px-8 py-4 font-bold tracking-widest hover:bg-white transition-colors flex items-center justify-center gap-2">
