export function BG3Cognac() {
  const bg = "#1e1006";
  const accent = "#FFB300";
  const text = "#fff8ee";
  const sub = "#c8964a";
  const muted = "rgba(200,150,74,0.5)";
  const label = "BG3 — Warm Cognac";
  return (
    <div className="min-h-screen flex flex-col" style={{ background: bg, fontFamily: "'Inter', sans-serif" }}>
      <nav className="flex items-center justify-between px-10 py-5 border-b" style={{ borderColor: "rgba(255,179,0,0.14)" }}>
        <div className="flex items-center gap-3">
          <div className="w-7 h-7 rounded-sm flex items-center justify-center" style={{ background: "rgba(255,179,0,0.15)", border: "1px solid rgba(255,179,0,0.35)" }}>
      ...
      <div className="flex-1 flex flex-col items-center justify-center text-center px-8 py-16 relative">
        <div className="absolute inset-0 pointer-events-none" style={{ background: `radial-gradient(ellipse 60% 50% at 50% 30%, rgba(255,179,0,0.08) 0%, transparent 70%)` }} />
        <div className="mb-6 px-4 py-1.5 rounded-sm border text-[9px] font-mono tracking-[0.4em]" style={{ borderColor: "rgba(255,179,0,0.25)", color: accent, background: "rgba(255,179,0,0.07)" }}>
