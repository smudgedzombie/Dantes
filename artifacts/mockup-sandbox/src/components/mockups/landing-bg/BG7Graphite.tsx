export function BG7Graphite() {
  const bg = "#1a1c22";
  const accent = "#FFB300";
  const text = "#f5f6f8";
  const sub = "#8892a4";
  const muted = "rgba(136,146,164,0.5)";
  const label = "BG7 — Graphite Smoke";
  return (
    <div className="min-h-screen flex flex-col" style={{ background: bg, fontFamily: "'Inter', sans-serif" }}>
      <nav className="flex items-center justify-between px-10 py-5 border-b" style={{ borderColor: "rgba(255,179,0,0.10)" }}>
        <div className="flex items-center gap-3">
          <div className="w-7 h-7 rounded-sm flex items-center justify-center" style={{ background: "rgba(255,179,0,0.12)", border: "1px solid rgba(255,179,0,0.28)" }}>
      ...
      <div className="flex-1 flex flex-col items-center justify-center text-center px-8 py-16 relative">
        <div className="absolute inset-0 pointer-events-none" style={{ background: `radial-gradient(ellipse 60% 50% at 50% 30%, rgba(255,179,0,0.05) 0%, transparent 70%)` }} />
        <div className="mb-6 px-4 py-1.5 rounded-sm border text-[9px] font-mono tracking-[0.4em]" style={{ borderColor: "rgba(255,179,0,0.18)", color: accent, background: "rgba(255,179,0,0.05)" }}>
