...
// Variant C — Muted Cream
// Bg: #181410 (warm dark brown), body text #d4bc78 (warm cream), most readable
...
const C = {
  bg: "#181410",
  cardBg: "#201a10",
  bodyText: "#d4bc78",       // warm cream — very readable
  subText: "#9a8548",
  mutedText: "#6a5e35",
  borderBase: "rgba(255,179,0,0.22)",
  borderHover: "rgba(255,179,0,0.45)",
  gridLine: "rgba(255,179,0,0.028)",
};

export default function VariantC() {
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: "white", fontFamily: "system-ui, sans-serif", overflowX: "hidden", position: "relative" }}>
      ...
      <nav style={{ position: "relative", zIndex: 20, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "20px 32px", borderBottom: `1px solid ${C.borderBase}` }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ display: "flex", flexDirection: "column", lineHeight: 1 }}>
            <span style={{ fontWeight: 900, color: "white", letterSpacing: "0.2em", fontSize: 18 }}>DANTÈS</span>
            <span style={{ fontSize: 9, fontFamily: "monospace", color: "rgba(255,179,0,0.70)", letterSpacing: "0.35em" }}>THE BLOOM SOCIETY</span>
          </div>
        </div>
      ...
      <section style={{ position: "relative", zIndex: 10, minHeight: "88vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", padding: "0 24px 48px" }}>
        <div style={{ position: "relative", zIndex: 20, display: "flex", flexDirection: "column", alignItems: "center" }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 12, padding: "8px 16px", border: "1px solid rgba(255,107,0,0.52)", background: "rgba(255,107,0,0.10)", borderRadius: 2, marginBottom: 32 }}>
          ...
          <div style={{ marginBottom: 12 }}>
            <span style={{ fontSize: 9, fontFamily: "monospace", color: "rgba(255,179,0,0.72)", letterSpacing: "0.4em" }}>THAILAND-BORN · GLOBALLY DEPLOYED</span>
          </div>

          <h1 style={{ fontSize: "clamp(5rem,12vw,10rem)", fontWeight: 900, color: "white", letterSpacing: "-0.02em", margin: "0 0 8px", lineHeight: 1, textShadow: "0 0 60px rgba(255,179,0,0.32)" }}>
            DANTÈS
          </h1>
          ...
          <p style={{ color: C.bodyText, maxWidth: 640, fontSize: 15, lineHeight: 1.7, marginBottom: 24 }}>
          ...
          <div style={{ display: "flex", gap: 24, marginBottom: 40, fontSize: 10, fontFamily: "monospace", color: C.subText, flexWrap: "wrap", justifyContent: "center" }}>
      ...
      <div style={{ position: "relative", zIndex: 10, borderTop: `1px solid ${C.borderBase}`, borderBottom: `1px solid ${C.borderBase}`, padding: "20px 32px", background: "rgba(24,20,16,0.95)" }}>
        <div style={{ maxWidth: 900, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 24, textAlign: "center" }}>
          {[
            ...
            <div key={s.label}>
              ...
              <div style={{ color: C.subText, fontSize: 10, fontFamily: "monospace" }}>{s.sub}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <section style={{ position: "relative", zIndex: 10, maxWidth: 900, margin: "0 auto", padding: "80px 32px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 48 }}>
          <div style={{ height: 1, flex: 1, background: C.borderBase }} />
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 24 }}>
          {[
            ...
            <div key={item.title} style={{ border: `1px solid ${C.borderBase}`, background: C.cardBg, borderRadius: 2, padding: 28 }}>
              <div style={{ fontSize: 28, marginBottom: 16 }}>{item.icon}</div>
              <h3 style={{ color: "white", fontWeight: 700, fontSize: 17, marginBottom: 12 }}>{item.title}</h3>
              <p style={{ color: C.bodyText, fontSize: 13, lineHeight: 1.7, margin: 0 }}>{item.body}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ position: "relative", zIndex: 10, maxWidth: 1100, margin: "0 auto", padding: "0 32px 48px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16, marginBottom: 24 }}>
          {AGENTS.map(a => (
            <div key={a.code} style={{ border: `1px solid ${C.borderBase}`, background: C.cardBg, borderRadius: 2, padding: 20 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
                <div>
                  ...
                  <div style={{ color: C.bodyText, fontSize: 11, fontFamily: "monospace" }}>{a.industry}</div>
                </div>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                {a.modules.map(m => (
                  <span key={m} style={{ fontSize: 10, fontFamily: "monospace", padding: "2px 8px", background: C.bg, border: `1px solid ${C.borderBase}`, color: C.bodyText, borderRadius: 2 }}>{m}</span>
                ))}
              </div>
            </div>
          ))}
        </div>
        <p style={{ textAlign: "center", color: C.subText, fontSize: 10, fontFamily: "monospace", letterSpacing: "0.15em" }}>EACH GRAHAM IS UNIQUELY CONFIGURED — NO TWO DEPLOYMENTS ARE IDENTICAL</p>
      </section>
      ...
      <section style={{ position: "relative", zIndex: 10, maxWidth: 1100, margin: "0 auto", padding: "0 32px 80px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16 }}>
          {MODULES.map(m => (
            <div key={m.id} style={{ border: `1px solid ${C.borderBase}`, background: C.cardBg, borderRadius: 2, padding: 20 }}>
              ...
              <div style={{ color: C.bodyText, fontSize: 12, lineHeight: 1.6 }}>{m.desc}</div>
            </div>
          ))}
        </div>
      </section>

      <footer style={{ position: "relative", zIndex: 10, borderTop: `1px solid ${C.borderBase}`, padding: "32px", textAlign: "center" }}>
        <div style={{ color: C.subText, fontSize: 10, fontFamily: "monospace", letterSpacing: "0.3em" }}>© 2026 THE BLOOM SOCIETY · DANTÈS FINANCE · ALL RIGHTS RESERVED</div>
      </footer>
    </div>
  );
}
